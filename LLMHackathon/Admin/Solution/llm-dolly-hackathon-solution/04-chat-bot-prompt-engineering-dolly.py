...
# MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&aip=1&t=event&ec=dbdemos&ea=VIEW&dp=%2F_dbdemos%2Fdata-science%2Fllm-dolly-chatbot%2F04-chat-bot-prompt-engineering-dolly&cid=984752964297111&uid=1814403842957890">
...
def build_qa_chain():
  ...
  # Because of hardware restrictions we use a very small LLM for the hackathon. 
  # This LLM is not competitive and is prone to hallucinations. 
  # So please keep in mind that bigger models (30B to 70B) generate much better results! 
  ...
  instruct_pipeline = pipeline("text-generation", model=instruct_model,trust_remote_code=True,  tokenizer=tokenizer, return_full_text=True, max_new_tokens=256, top_p=0.95, top_k=50)
  ...
  # We could use one of these models: https://huggingface.co/models?filter=summarization. facebook/
  # bart-large-cnn gives great results, we'll use t5-small for memory
  summarize_model_path = "/dbfs/FileStore/cs_hackathon/instruction/t5-small/"


  # TODO
  # Load AutoModelForSeq2SeqLM and AutoTokenizer from summarize_model_path and build pipeline from it. 

  # Hint:
  # Check the building pattern for the instruction model a few lines on top. 

  # Answer: 
  summarize_model = AutoModelForSeq2SeqLM.from_pretrained(summarize_model_path, device_map="auto", torch_dtype=torch.float16)
  summarize_tokenizer = AutoTokenizer.from_pretrained(summarize_model_path, padding_side="left", model_max_length = 512)
  pipe_summary = pipeline("summarization", model=summarize_model, tokenizer=summarize_tokenizer) 
  

  
  #, max_new_tokens=500, min_new_tokens=300
...
# MAGIC %md
# MAGIC
# MAGIC ### Following chapter is optional for the hackathon
# MAGIC -----------------------------------------------------------------------------
